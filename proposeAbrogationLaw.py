...
for i in range(len(lawIds)):
    for versionNumber in acceptedLaws[lawIds[i]]:
        print(str(i)+'. '+acceptedLaws[lawIds[i]][versionNumber]['content'])
...
pttp.proposeAbrogationLaw(username, lawToAbrogateId, replace, replacementLaw)
